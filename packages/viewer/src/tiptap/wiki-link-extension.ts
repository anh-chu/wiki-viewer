...
} from "./wiki-link-decorator";

export type { WikiSlugResolver };
...
/**
 * Create a WikiLink extension wired to an optional slug resolver.
 * The resolver is passed through to the decorator plugin so it can
 * mark broken (unknown) wiki-links. When absent, all wiki-links
 * render as unknown (data-broken="true").
 */
export function createWikiLink(resolver?: WikiSlugResolver) {
	return WikiLink.extend({
		addProseMirrorPlugins() {
			return [wikiLinkDecoratorPlugin(resolver)];
		},
	});
}
