...
export function deleteProposal(previewId: string): void { getDb().prepare(`DELETE FROM md_proposal WHERE previewId=? AND state='requested' AND request_id IS NULL`).run(previewId); }
