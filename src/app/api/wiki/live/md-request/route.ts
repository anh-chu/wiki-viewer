...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  if (typeof path !== "string" || typeof blockRef !== "string" || typeof instruction !== "string" || !instruction.trim() || typeof b.baseRevision !== "number" || typeof b.baseBlockHash !== "string") return NextResponse.json({error:"INVALID_PARAM",message:"path, blockRef, baseRevision, baseBlockHash, instruction required"},{status:400});
  ...
  const proposal = createProposal({workspaceId:ctx.ws.id,path:relPath,blockRef,baseRevision:b.baseRevision,baseBlockHash:b.baseBlockHash});
  const enq = enqueueRequest({sessionId:session.id,workspaceId:ctx.ws.id,path:relPath,blockRef,baseRevision:b.baseRevision,kind:"generate",instruction,selectionText:typeof b.selectionText === "string" ? b.selectionText : null,selectionStart:typeof b.selectionStart === "number" ? b.selectionStart : null,selectionEnd:typeof b.selectionEnd === "number" ? b.selectionEnd : null});
