...
import { findEnclosingGitRepo, gitFileInfo } from "@/lib/git";
...
export async function GET(request: Request) {
	...
	if (!repo) return NextResponse.json({ info: null });

	try {
		const info = await gitFileInfo(repo.repoDir, repo.relFromRepo);
