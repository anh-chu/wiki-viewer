...
import { FileViewer, fileKindFor, type FileKind } from "@wiki-viewer/viewer";
...
// ── File type detection ──────────────────────────────────────────────────────
// Owned by @wiki-viewer/viewer so this page and an external host classify
// identically. fileKindFor is the former local sharedFileKind.
...
export default function SharedPage({
	...
	// The header's ViewWidthToggle drives these; the viewer takes them as props
	// instead of reaching into the store, so the package stays store-free.
	...
	const kind = useMemo<FileKind>(() => {
		...
		return fileKindFor(state.filename);
	}, [state]);
			...
			<div className="min-h-screen flex flex-col bg-background text-foreground">
				...
				{state.kind === "content" && (
					<FileViewer
						...
						assetUrl={`/api/share/${token}/asset`}
						assetUrlTransform={withWs}
						maxWidth={editorMaxW}
						marginLeft={editorMl}
					/>
				)}
