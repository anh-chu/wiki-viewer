...
export function SuggestionPip({ top, left, count, onClick, "aria-label": ariaLabel, anchorKey }: Props) {
	...
	return <>
		<span aria-hidden="true" data-annotation-span={anchorKey ?? ariaLabel} className={`pointer-events-none absolute z-[1] rounded text-transparent transition-colors ${hovered ? "bg-primary/5" : ""}`} style={{ top, left: Math.max(0, left + 20), width: "min(32rem, calc(100vw - 4rem))", minHeight: 20 }} />
