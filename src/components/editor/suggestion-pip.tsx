...
interface Props { top: number; left: number; count: number; onClick: MouseEventHandler<HTMLButtonElement>; "aria-label": string; anchorKey?: string; active?: boolean }

/** Gutter pip with a co-located, queryable span used for review anchoring. */
export function SuggestionPip({ top, left, count, onClick, "aria-label": ariaLabel, anchorKey, active = false }: Props) {
	...
	}, [resolvedAnchorKey, lit, top, left]);
	const highlightStyle = highlightRect ?? { top, left: Math.max(0, left + 20), width: "min(32rem, calc(100vw - 4rem))", minHeight: 20 };
	return <>
		<span aria-hidden="true" data-annotation-span={resolvedAnchorKey} className={`pointer-events-none absolute z-[1] rounded text-transparent transition-colors ${lit ? "bg-success/15" : ""}`} style={highlightStyle} />
