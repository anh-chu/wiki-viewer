...
interface Props { anchorKey: string; anchorLabel?: string; comments: Comment[]; top: number; left: number; onClick: () => void; variant?: "comment" | "instruction"; active?: boolean }

/** Gutter pip with a co-located, queryable span used for thread anchoring. */
export function CommentPip({ anchorKey, anchorLabel, comments, top, left, onClick, variant = "comment", active = false }: Props) {
	...
	if (variant === "instruction") {
		return <>
			<span aria-hidden="true" data-annotation-span={anchorKey} className={`pointer-events-none absolute z-[1] rounded text-transparent transition-colors ${lit ? "bg-amber-500/15" : ""}`} style={highlightStyle}>{anchorLabel ?? anchorKey}</span>
	...
	return <>
		<span aria-hidden="true" data-annotation-span={anchorKey} className={`pointer-events-none absolute z-[1] rounded text-transparent transition-colors ${lit ? "bg-primary/10" : ""}`} style={highlightStyle}>{anchorLabel ?? anchorKey}</span>
