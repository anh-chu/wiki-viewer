...
export function CommentPip({ anchorKey, anchorLabel, comments, top, left, onClick, variant = "comment" }: Props) {
	...
	if (variant === "instruction") {
		return <>
			<span aria-hidden="true" data-annotation-span={anchorKey} className={`pointer-events-none absolute z-[1] rounded text-transparent transition-colors ${hovered ? "bg-amber-500/5" : ""}`} style={{ top, left: Math.max(0, left + 20), width: "min(32rem, calc(100vw - 4rem))", minHeight: 20 }}>{anchorLabel ?? anchorKey}</span>
