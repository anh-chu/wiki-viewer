...
**Fast path (same machine only):** when running directly on the wiki-viewer host, send your registration with the header `X-Service-Token: <contents of ~/.wiki-viewer/service-token>` (0600, written by the server at startup, shown/rotated in Settings). Registration then auto-approves: the response is `200 {status:"approved", agentId, token}` immediately — no pending state, no polling, no approval step. Requested scope is granted as-is. Tell the human only if the fast path is rejected.
...
(With a service-token header you won't reach this step — see the fast path above.)
...
2. POST /api/agent/register                              # register (include "delete" in ops if needed)
   # on the server host? add X-Service-Token -> token comes back instantly, skip 3-4
